...
class LinearPatchEmbedding(Patchify):
    ...
    def forward(self, x, split_by_time=False, verbose = False, **kwargs):
        x = super().forward(x, **kwargs)
        if verbose: print(f"Patchify Size: {list(x.shape)}")
        x = self.embed(x)
        if verbose: print(f"Embedding Size: {list(x.shape)}")
        if split_by_time:
